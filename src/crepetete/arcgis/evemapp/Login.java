...
public class Login extends Activity {
	...
	// Een groot deel van deze Activity komt van de Facebook tutorial op in te
	// loggen.
	...
	// In deze functie halen we de informatie van de gebruiker op.
					...
					public void onCompleted(GraphUser user, Response response) {
						if (user.getId() != null && user.getName() != null) {
							...
							if (friendRequestCompleted) {
								// Als de functie klaar is starten we de MainMap
								startMainMap(user);
							}
						} else {
	...
	// Getting friend information. (deels gevonden op
	// http://stackoverflow.com/questions/13957181/get-friend-list-facebook-3-0)
	private void getFriends() {
		Request friendRequest = Request.newMyFriendsRequest(
				Session.getActiveSession(), new GraphUserListCallback() {
					public void onCompleted(List<GraphUser> users,
							Response response) {
						// als de request gelukt is, maak ik er iets makkelijk
						// leesbaars van
						friendsList = parseUserFromFQLResponse(response);
						// om het netjes te houden heb ik ervoor gekozen om de
						// namen te sorteren
		...
		// http://stackoverflow.com/questions/13984873/android-how-to-get-friends-birth-dates-from-facebook-calendar-using-graph-api
		// om de juiste params te vinden.
	...
	// hier parse ik de response van de request naar info van vrienden.
	// (geholpen door
	// http://stackoverflow.com/questions/14422718/how-to-read-fql-response-in-android)
	private ArrayList<Friend> parseUserFromFQLResponse(Response response) {
		...
		try {
			...
			for (int i = 0; i < (arr.length()); i++) {
				...
				// als er info meegegeven is, sla het op en stop het in een
				// Friend object, die op zijn beurt weer in een friendList wordt
				// gestopt,
				// zodat de list later kan worden gesorteerd en er makkelijker
				// een adapter voor gemaakt kan worden om de info in de ListView
				// te zetten.
				// als de info niet ingevoerd is door de vriend of niet is
				// opgehaald, zal de app 'undefined' aangeven voor naam en
				// verjaardag. Voor werk geeft hij '' weer,
				// omdat iemand geen werk kan hebben, maar niemand heeft geen
				// naam of verjaardag.
				String id = "undefined";
